...
{
    class IntersectingBoundaryEdgeRayEmitter
    {
        public Point pos;
        ...
        public IntersectingBoundaryEdgeRayEmitter(double x, double y)
        {
            pos = new Point(x, y);
        ...
        public void Update(Boundary[] boundaries)
        {
            ...
            // Add a POI for each intersection of boundaries
            if (boundaries.Length > 1)
            {
                for (int i = 0; i < boundaries.Length - 1; i++)
                {
                    ...
                    {
                        Point? pt = boundaries[i].Intersects(boundaries[j]);
                        if (pt != null)
                        {
                            pointsOfInterest.Add(pt.Value);
                        }
                    }
                }
            }
            ...
            foreach (Ray ray in rays)
            {
                ...
                if (closest != null)
                {
                    ...
                    collisionPoints.Add(closest.Value);
                }
            }
        }
