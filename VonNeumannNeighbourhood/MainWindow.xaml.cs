...
namespace VonNeumannNeighbourhood
{
    /// <summary>
    ...
    public partial class MainWindow : DrawingWindowBase
    {
        ...
        private readonly int iterations = 95;
        ...
        public override void Update(float dt)
        {
            if (iteration < iterations)
            {
                while (cellsToCheck.Count > 0 && cellsToCheck.Peek().iteration == iteration)
                {
                    ...
                    if (HasExactly1ActiveNeighbour(iteration, cell.x, cell.y, out List<Cell> inactiveNeighbours))
                    {
        ...
        private Queue<Cell> InitializeQueue()
        {
            ...
            for (int y = 0; y < currentState.GetLength(1); y++)
            {
                for (int x = 0; x < currentState.GetLength(0); x++)
                {
                    if (currentState[x, y])
                    {
                        foreach (Cell c in GetInactiveNeighbours(-1, x, y))
                        {
                            queue.Enqueue(c);
                        }
                    }
        ...
        private List<Cell> GetInactiveNeighbours(int iteration, int x, int y)
        {
        ...
        private bool HasExactly1ActiveNeighbour(int iteration, int x, int y, out List<Cell> inactiveNeighbours)
        {
            List<Cell> tmp = GetInactiveNeighbours(iteration, x, y);
